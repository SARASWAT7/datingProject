...
class Politices extends StatefulWidget {
  final String selectedPolitic;

  const Politices({Key? key, this.selectedPolitic = ""}) : super(key: key);

  @override
  State<Politices> createState() => _PoliticesState();
}

class _PoliticesState extends State<Politices> {
  final List<String> politices = [
    "Apolitical",
    "Socialist",
    "Communist",
    "Feudal",
  ...
  void initState() {
    ...
    selectedButton = widget.selectedPolitic;
    // Pre-select the button based on the passed politic value
    controller.selectIndex(politices.indexOf(selectedButton));
  }

  void _savePolitices() {
    if (selectedButton.isEmpty) {
      showToast(context, 'Please select a political stance.');
      return;
    }
    final formData = FormData.fromMap({
      'politic': selectedButton,
    });
