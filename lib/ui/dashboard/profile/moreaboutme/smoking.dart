...
class Smokeing extends StatefulWidget {
  final String smoking;
  const Smokeing({Key? key, this.smoking = ""}) : super(key: key);

  @override
  State<Smokeing> createState() => _SmokeingState();
}

class _SmokeingState extends State<Smokeing> {
  final List<String> smokeOptions = [
    'Never', 'Socially', 'Frequently', 'Sober'
  ];
  ...
  void initState() {
    super.initState();
    selectedButton = widget.smoking;
    controller.selectIndex(smokeOptions.indexOf(selectedButton));
  }

  void _saveSmoking() {
    if (selectedButton.isEmpty) {
      showToast(context, 'Please select a smoking preference.');
      return;
    }
    final formData = FormData.fromMap({
      'smoking': selectedButton,
    });
