...
class Kids extends StatefulWidget {
  ...
  const Kids({Key? key, this.haveKids = ""}) : super(key: key);
...
class _KidsState extends State<Kids> {
  ...
  void initState() {
    ...
    selectedButton = widget.haveKids;
    // Pre-select the button based on the passed haveKids value
    controller.selectIndex(kidsOptions.indexOf(selectedButton));
  }

  void _saveKids() {
    if (selectedButton.isEmpty) {
      showToast(context, 'Please select a kids status.');
      return;
    }
    final formData = FormData.fromMap({
      'have_kids': selectedButton,
    });
