...
class ContactUsCubit extends Cubit<ContactUsState> {
  ...
  void contactus(BuildContext context, String message) async {
    AppUtils.keyboardHide(context);
    if (message.isEmpty) {
      NormalMessage().normalerrorstate(context, NormalMessage().contactUs);
    } else {
      CorettaUserProfileRepo repo = CorettaUserProfileRepo();
      emit(state.copyWith(status: ApiState.isLoading));
      try {
        final response = await repo.contactus(context, message);
